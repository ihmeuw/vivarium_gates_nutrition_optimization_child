...
def get_daily_mam_remission_probability(
    builder: Builder, index: pd.Index, mam_tx_coverage: float, mam_tx_efficacy: float
) -> pd.Series:
    draw = builder.configuration.input_data.input_draw_number
    mam_tx_recovery_time = pd.Series(index=index, name="mam_remission")
    mam_tx_recovery_time[
        index.get_level_values("age_start") < 0.5
    ] = data_values.WASTING.MAM_TX_RECOVERY_TIME_UNDER_6MO
    mam_tx_recovery_time[0.5 <= index.get_level_values("age_start")] = get_random_variable(
        draw, *data_values.WASTING.MAM_TX_RECOVERY_TIME_OVER_6MO
    )
...
def get_daily_sam_incidence_probability(
    ...
    i1 = (
        ...
        + adj_exposures[WASTING.CAT1]
        * untreated_sam_remission_prob
        / adj_exposures[WASTING.CAT2]
        + adj_exposures[WASTING.CAT1]
        * treated_sam_remission_prob
        / adj_exposures[WASTING.CAT2]
...
def get_daily_sam_untreated_remission_probability(
    ...
    sam_mortality_rate = _convert_daily_probability_to_annual_rate(
        mortality_probs[WASTING.CAT1]
    )
...
def load_daily_mortality_probabilities(builder: Builder) -> pd.DataFrame:
    ...
    emr_c = pd.concat(
        [get_data_series(builder.data.load(c.EMR)).rename(c.name) for c in causes], axis=1
    )
    ...
    # index = [ 'sex', 'age_start', 'age_end', 'year_start', 'year_end', 'affected_entity' ]
    csmr_c = pd.concat(
        [get_data_series(builder.data.load(c.CSMR)).rename(c.name) for c in causes], axis=1
    )
    ...
    # paf_c
    # index = [ 'sex', 'age_start', 'age_end', 'year_start', 'year_end', 'affected_entity' ]
    paf_c = get_data_series(builder.data.load(WASTING.PAF)).droplevel("affected_measure")

    # rr_ci
    # index = [
    #   'sex', 'age_start', 'age_end', 'year_start', 'year_end', 'affected_entity', 'parameter
    # ]
    rr_ci = get_data_series(builder.data.load(WASTING.RELATIVE_RISK)).droplevel(
        "affected_measure"
    )
    ...
    duration_c.loc[
        duration_c.index.get_level_values("age_start") == 0.0
...
def get_data_series(data: pd.DataFrame) -> pd.Series:
    return data.set_index(data.columns[:-1].to_list()).squeeze()
