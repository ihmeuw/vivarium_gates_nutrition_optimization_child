...
from vivarium_gates_nutrition_optimization_child.constants import data_keys, data_values, models, scenarios
...
class SQLNSTreatment:
    ...
    def setup(self, builder: Builder):
        ...
        self.tmrel_to_mild_wasting_risk_ratio = self.get_risk_ratios(builder, 'tmrel_to_mild_rate')
        self.mild_to_mam_wasting_risk_ratio = self.get_risk_ratios(builder, 'mild_to_mam_rate')
        self.mam_to_sam_wasting_risk_ratio = self.get_risk_ratios(builder, 'mam_to_sam_rate')

        self.severe_stunting_risk_ratio = self.get_risk_ratios(builder, 'severe_stunting_prevalence_ratio')
        self.moderate_stunting_risk_ratio = self.get_risk_ratios(builder, 'moderate_stunting_prevalence_ratio')
    ...
    def get_risk_ratios(self, builder: Builder, affected_outcome: str) -> LookupTable:
        ...
        risk_ratios = risk_ratios.query("affected_outcome==@affected_outcome").drop('affected_outcome', axis=1)
    ...
    def get_current_coverage(self, index: pd.Index) -> pd.Series:
        ...
        covered = ((propensity < data_values.SQ_LNS.COVERAGE_BASELINE)
    ...
    def apply_tmrel_to_mild_wasting_treatment(self, index: pd.Index, target: pd.Series) -> pd.Series:
        covered = self.coverage(index) == 'covered'
        target[covered] = target[covered] * self.tmrel_to_mild_wasting_risk_ratio(index)

        return target

    def apply_mild_to_mam_wasting_treatment(self, index: pd.Index, target: pd.Series) -> pd.Series:
        covered = self.coverage(index) == 'covered'
        target[covered] = target[covered] * self.mild_to_mam_wasting_risk_ratio(index)

        return target

    def apply_mam_to_sam_wasting_treatment(self, index: pd.Index, target: pd.Series) -> pd.Series:
        covered = self.coverage(index) == 'covered'
    ...
    def apply_stunting_treatment(self, index: pd.Index, target: pd.DataFrame) -> pd.DataFrame:
        ...
        target.loc[covered, 'cat4'] = (target.loc[covered, 'cat4']
