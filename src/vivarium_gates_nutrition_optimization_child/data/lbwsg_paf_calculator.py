...
def get_relative_risks(
    config: Path, artifact_path: str, input_draw: int, random_seed: int, age_group_id: int
) -> pd.DataFrame:
    logger.remove()
    ...
    sim.configuration.update(
        {
            ...
            "population": {"age_start": age_start, "age_end": age_end},
        }
    )
    sim.setup()
    ...
    def calculate_rr_by_sex(sex: str) -> float:
        ...
        interpolator = pickle.loads(
            bytes.fromhex(interpolators.loc[row_index, f"draw_{input_draw}"])
        )
        rrs = np.exp(
            interpolator(gestational_ages[sex_mask], birth_weights[sex_mask], grid=False)
        )
        return rrs

    lbwsg_rrs = pd.DataFrame({"relative_risk": 1.0}, index=pop.index)
    lbwsg_rrs["sex"] = pop["sex"]
    lbwsg_rrs.loc[lbwsg_rrs["sex"] == "Female", "relative_risk"] = calculate_rr_by_sex(
        "Female"
    )
...
def get_age_bin(config: Path, artifact_path: str, age_group_id: int) -> pd.Interval:
    ...
    age_bins = age_bins.reset_index().set_index("age_group_id")
    age_bin = pd.Interval(
        age_bins.loc[age_group_id, "age_start"], age_bins.loc[age_group_id, "age_end"]
    )

    return age_bin


def get_paf_for_age_group(
    config: Path, artifact_path: str, input_draw: int, random_seed: int, age_group_id: int
) -> pd.DataFrame:
    age_bin = get_age_bin(config, artifact_path, age_group_id)
    relative_risks = pd.concat(
        [
            get_relative_risks(config, artifact_path, input_draw, seed, age_group_id)
...
def write_pafs_to_hdf(
    config: str, artifact_path: str, output_dir: str, input_draw: str, random_seed: str
):
    config = Path(config)
    ...
    pafs = pd.concat(
        [
            get_paf_for_age_group(
                config, artifact_path, input_draw, random_seed, age_group_id
            )
