...
def get_data(
    ...
    mapping = {
        ...
        data_keys.UNCOMPLICATED_SEVERE_PEM.RESTRICTIONS: load_pem_restrictions,
        data_keys.COMPLICATED_SEVERE_PEM.EMR: load_pem_emr,
        data_keys.COMPLICATED_SEVERE_PEM.CSMR: load_pem_csmr,
        data_keys.COMPLICATED_SEVERE_PEM.RESTRICTIONS: load_pem_restrictions,
...
def load_wasting_transition_rates(key: str, location: Union[str, List[int]]) -> pd.DataFrame:
    ...
    # update remission transition values
    rates = rates.set_index(metadata.ARTIFACT_INDEX_COLUMNS + ["transition"]).sort_index()
...
def load_gbd_2021_exposure(key: str, location: Union[str, List[int]]) -> pd.DataFrame:
    ...
    cat1_super = data.query("parameter=='cat1'").copy().droplevel("parameter")

    # cat1_complicated = cat1_superstate * comp_frac
    cat1_complicated_rows = (
        cat1_super[metadata.ARTIFACT_COLUMNS] * comp_frac[metadata.ARTIFACT_COLUMNS]
    )
    ...
    cat1_uncomplicated_rows = (
        cat1_super[metadata.ARTIFACT_COLUMNS] * (1 - comp_frac[metadata.ARTIFACT_COLUMNS])
    )
...
def _load_complicated_sam_fraction(national_location_id: int) -> pd.DataFrame:
    ...
    The CSV only has post-neonatal ages; neonatal ages are filled
    with NaN.
    """
    rates = pd.read_csv(
        paths.WASTING_TRANSITIONS_COMPLICATED_SAM_DATA_DIR / f"{national_location_id}.csv"
    ...
    # Add neonatal age groups with NaN draw values
...
def load_sam_treatment_rr(key: str, location: str) -> pd.DataFrame:
    ...
    rr_sam_treated_remission[
        "affected_entity"
    ] = "uncomplicated_severe_acute_malnutrition_to_mild_child_wasting"
    ...
    better_mam_rows[
        "affected_entity"
    ] = "uncomplicated_severe_acute_malnutrition_to_better_moderate_acute_malnutrition"
    worse_mam_rows[
        "affected_entity"
    ] = "uncomplicated_severe_acute_malnutrition_to_worse_moderate_acute_malnutrition"
